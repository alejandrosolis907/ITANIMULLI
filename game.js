...
(() => {
  ...
  let nextStar = 50; // estrella flotante en ciclo 2 cada 50s
  ...
  function clearEnemies(){
    missiles.length = reptiles.length = triangles.length = lasers.length = angels.length = eyes.length = dollars.length = lifeStars.length = 0;
    cosmics.length = 0;
  }
  ...
  class Star {
    ...
    update(dt) {
      this.x -= this.speed * dt;
      this.t += dt;
      this.y = this.baseY + Math.sin(this.t * 2) * 10;
      if (this.x < -this.w) this.alive = false;
    }
    render(ctx) {
      ctx.save();
      ...
      for (let i=0;i<5;i++) {
        ...
        ctx.lineTo(Math.cos((54 + i*72)*Math.PI/180)*this.r*0.5,
                  -Math.sin((54 + i*72)*Math.PI/180)*this.r*0.5);
      }
      ctx.closePath();
      ctx.fill();
      ctx.restore();
  ...
  function resetAndStart(preserve=false){
    state = STATE.PLAY;
    missiles.length = reptiles.length = triangles.length = lasers.length = angels.length = eyes.length = dollars.length = lifeStars.length = scenery.length = cosmics.length = 0;
  ...
  function update(dt){
    ...
    lifeStars.forEach(o=>o.update(dt));
    ...
    lifeStars.forEach(s => {
      if (s.alive && rectsOverlap(pb, s.bbox())) {
        s.alive = false;
        lives += 1;
        livesEl.textContent = lives;
        lifeTipUntil = time + 1.5;
